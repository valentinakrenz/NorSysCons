function compTime=Compute_trialUnique_ENS_Searchlight_fishertransformedr(sj,radius)
...
for sl=1:numel(LinearInds)
    ...
    SL_Patts=SelDat(:,slInds);
     ...
     ENS=mean(EncNewRDM(HypMat==1));%trial-unique (on-diagonal)
     ENSPerSL(LinearInds(sl))=ENS;
     ENSPerSL_fishertransformed=atanh(ENSPerSL);
end

%%original
save(['SL_outputMaps_trialUnique\ENSPerSL_fishertransformed_Rad' num2str(radius) '_SJ_timages' num2str(sj) '.mat'],'ENSPerSL_fishertransformed','-v7.3');
nii_template=load_nii('beta_0001.nii');
nii_template.img=ENSPerSL_fishertransformed;
save_nii(nii_template,['SL_outputMaps_trialUnique\ENSPerSL_fishertransformed_Rad' num2str(radius) '_SJ_timages' num2str(sj) '.nii']);
compTime=toc