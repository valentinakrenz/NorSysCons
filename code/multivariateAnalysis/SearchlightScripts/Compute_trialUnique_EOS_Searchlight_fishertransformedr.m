function compTime=Compute_trialUnique_EOS_Searchlight_fishertransformedr(sj,radius)
...
SelDat=BetaMaps([121:180 181:240],:,:,:);
...
for sl=1:numel(LinearInds)
    ...
    SL_Patts=SelDat(:,slInds);
     ...
     EOS=mean(EncNewRDM(HypMat==1));%trial-unique (on-diagonal)
     EOSPerSL(LinearInds(sl))=EOS;
     EOSPerSL_fishertransformed=atanh(EOSPerSL);
end

%%original
save(['SL_outputMaps_trialUnique\EOSPerSL_fishertransformed_Rad' num2str(radius) '_SJ_timages' num2str(sj) '.mat'],'EOSPerSL_fishertransformed','-v7.3');
nii_template=load_nii('beta_0001.nii');
nii_template.img=EOSPerSL_fishertransformed;
save_nii(nii_template,['SL_outputMaps_trialUnique\EOSPerSL_fishertransformed_Rad' num2str(radius) '_SJ_timages' num2str(sj) '.nii']);
compTime=toc